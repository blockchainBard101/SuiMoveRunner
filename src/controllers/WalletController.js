...
class WalletController extends BaseController_1.BaseController {
    async handleSwitchWallet(message) {
        ...
        try {
            await (0, shell_1.runCommand)(`sui client switch --address ${address}`);
    ...
    async handleCreateAddress() {
        try {
            const keyScheme = await vscode.window.showQuickPick(["ed25519", "secp256k1", "secp256r1"], { placeHolder: "Select key scheme for new address" });
            if (!keyScheme) {
            ...
            await this.state.refreshWallets();
            vscode.window.showInformationMessage(`✅ New address created:\nAlias: ${parsed.alias}\nAddress: ${parsed.address}\nRecovery Phrase: ${parsed.recoveryPhrase}`);
            this.postMessage("refresh");
        }
        catch (err) {
    ...
    async handleImportWallet(message) {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`❌ Failed to import wallet: ${err}`);
    ...
    async handleExportWallet() {
        try {
            ...
            const warningAction = await vscode.window.showWarningMessage(`⚠️ Security Warning: Exporting Private Key\n\n` +
                ...
                `Are you sure you want to continue?`, { modal: true }, "Yes, I understand the risks", "Cancel");
            ...
            const output = await (0, shell_1.runCommand)(`sui keytool export --key-identity ${keyIdentity} --json`);
            ...
            const action = await vscode.window.showInformationMessage(`✅ Private key exported for wallet ${walletAddress.slice(0, 6)}...${walletAddress.slice(-4)}`, "Copy to Clipboard", "Show Details");
            if (action === "Copy to Clipboard") {
                ...
                const panel = vscode.window.createWebviewPanel('walletExport', 'Wallet Export Details', vscode.ViewColumn.One, { enableScripts: true });
                panel.webview.html = `
            <!DOCTYPE html>
            <html>
            <head>
          ...
          `;
            ...
            vscode.window.showErrorMessage(`❌ Failed to export wallet: ${err}`);
        }
    }
    async handleViewCoinPortfolio() {
        try {
            ...
            vscode.window.showInformationMessage("💰 Coin portfolio updated!");
        }
        catch (err) {
            vscode.window.showErrorMessage(`❌ Failed to fetch coin portfolio: ${err}`);
        }
    }
    handleShowGasCoinCopyNotification(message) {
        const coinId = message.coinId;
        const shortId = message.shortId || coinId.slice(0, 8) + "..." + coinId.slice(-8);
        vscode.window.showInformationMessage(`📋 Gas coin ID copied to clipboard: ${shortId}`);
    }
    ...
    async getKeyIdentityForAddress(address) {
        try {
            // Get all addresses to find the key identity
            const output = await (0, shell_1.runCommand)('sui client addresses --json');
            const parsed = (0, parsing_1.safeJsonParse)(output);
            ...
            if (parsed.addresses && Array.isArray(parsed.addresses)) {
                const wallet = parsed.addresses.find((addr) => addr[1] === address);
