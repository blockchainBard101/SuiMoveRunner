...
class PackageController extends BaseController_1.BaseController {
    async handleCreate(message) {
        ...
        if (!packageNameRegex.test(name)) {
            vscode.window.showErrorMessage("Invalid package name. Please use only letters, numbers, and underscores, and start with a letter.");
        ...
        try {
            await (0, shell_1.runCommand)(`${this.state.suiPath} move new ${name}`, basePath);
            await (0, FileService_1.waitForFolder)(targetPath, 2000);
    ...
    async handleBuild() {
        ...
        if (isEphemeralEnv && fs.existsSync(moveTomlPath)) {
            try {
                ...
                if (chainId) {
                    const result = (0, parsing_1.surgicalUpdateToml)(moveTomlContent, "environments", activeEnv, chainId, true);
                    if (result.changed) {
        ...
        vscode.window.showInformationMessage(`🛠️ Running '${buildSuiCmd}' in ${rootPath}${envNote}...`);
    }
    async handlePublish() {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
        ...
        if (isEphemeralEnv) {
            outputChannel.appendLine(`🌐 Environment: ${activeEnv} (ephemeral network — wiped weekly)\n` +
        ...
        try {
            ...
            const publishProcess = (0, child_process_1.exec)(publishCmd, {
                cwd: rootPath,
            ...
            publishProcess.on("close", async (code) => {
                if (code === 0) {
                    if (isEphemeralEnv) {
                        ...
                        vscode.window.showInformationMessage(`✅ test-publish succeeded on ${activeEnv}. Move.toml unchanged (ephemeral).`);
                        ...
                        if (pkg && fs.existsSync(moveTomlPath)) {
                            try {
                                ...
                                const pubResult = (0, parsing_1.surgicalUpdateToml)(newContent, "package", "published-at", pkg, true);
                                ...
                                if (pkgName) {
                                    const addrResult = (0, parsing_1.surgicalUpdateToml)(newContent, "addresses", pkgName, pkg, false);
    ...
    async handleUpgrade(message) {
        ...
        try {
            ...
            const envData = upgradeData.environments?.[this.state.activeEnv] || upgradeData.upgrade; // fallback to old format
            if (!envData) {
                vscode.window.showErrorMessage(`⚠️ No upgrade capability found for environment: ${this.state.activeEnv}`);
                return;
            }
            upgradeCapInfo = {
        ...
        outputChannel.show(true);
        outputChannel.appendLine(`Running '${this.state.suiPath} client upgrade --upgrade-capability ${upgradeCapInfo.upgradeCap}' in ${rootPath}...\n`);
        try {
            const isWindows = process.platform === 'win32';
            const upgradeProcess = (0, child_process_1.exec)(`${this.state.suiPath} client upgrade --upgrade-capability ${upgradeCapInfo.upgradeCap}`, {
                cwd: rootPath,
                shell: isWindows ? 'cmd.exe' : undefined,
            });
            let fullOutput = "";
    ...
    async handleScanMoveProjects() {
        try {
            ...
            if (this.state.foundMoveProjects.length === 0) {
                vscode.window.showInformationMessage("No Move projects found in the current workspace.");
            }
            else {
                vscode.window.showInformationMessage(`Found ${this.state.foundMoveProjects.length} Move project(s). Check the sidebar to select one.`);
            }
        }
        catch (err) {
            this.setStatus("");
    ...
    async handleSelectMoveProject(message) {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`✅ Selected Move project: ${projectName}`);
        }
        catch (err) {
    ...
    async handleResetDeployment() {
        ...
        const warningAction = await vscode.window.showWarningMessage("⚠️ Danger Zone: Reset Deployment\n\n" +
            "This will DELETE 'Move.lock' and 'Publish.toml', and remove published addresses from 'Move.toml'.\n\n" +
            "Are you sure you want to proceed?", { modal: true }, "Yes, Reset Deployment", "Cancel");
    ...
    async handlePublishWithDeps() {
        ...
        const publishProc = (0, child_process_1.exec)(cmd, { cwd: rootPath, shell: isWindows ? 'cmd.exe' : undefined });
        publishProc.stdout?.on("data", (data) => outputChannel.append(data.toString()));
        publishProc.stderr?.on("data", (data) => outputChannel.append(data.toString()));
    ...
    async handleAddDependency(message) {
        ...
        try {
            ...
            switch (depType) {
                case "mvr":
                    ...
                    if (network)
                        mvrParts.push(`network = "${network}"`);
                    depValueStr = `{ ${mvrParts.join(", ")} }`;
                    break;
                case "git":
                    const gitParts = [`git = "${value}"`];
                    if (subdir)
                        gitParts.push(`subdir = "${subdir}"`);
                    if (rev)
                        gitParts.push(`rev = "${rev}"`);
            ...
            const result = (0, parsing_1.surgicalUpdateToml)(content, "dependencies", name, depValueStr, false);
            if (result.changed) {
                fs.writeFileSync(moveTomlPath, result.content);
    ...
    ensureEphemeralGitignore(rootPath, outputChannel) {
